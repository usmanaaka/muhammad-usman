...
//			});
		...
		contactForm: function(){
			document.querySelector('.awilo_fn_contact_sendbtn').addEventListener('click', function (e) {
				...
				if (!name ) {
					document.querySelector('.empty_notice').style.display = 'block'; // Show empty notice
					return;
				} else {
					document.querySelector('.empty_notice').style.display = 'none'; // Hide empty notice
				}
			
				// Encode the message for WhatsApp
				const whatsappMessage = encodeURIComponent(`Hello, I am ${name}. My email is ${email}. Here is my message: ${message}`);
			
				// WhatsApp URL with the encoded message
				const whatsappUrl = `https://wa.me/923353398374?text=${whatsappMessage}`;
